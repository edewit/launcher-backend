...
public class CouldServiceProducerTest {
    ...
    public void should_authenticate_using_header_value() {
        ...
        when(request.getHeader(CouldServiceProducer.OPENSHIFT_AUTHORIZATION_HEADER)).thenReturn("Bearer bar");
        TokenIdentity auth = TokenIdentity.of("foo");
        TokenIdentity authFromHeader = TokenIdentity.of("bar");
        CouldServiceProducer producer = new CouldServiceProducer(factory, clusterRegistry);
    ...
    public void should_authenticate_using_identity_provider() {
        TokenIdentity auth = TokenIdentity.of("foo");
        when(clusterRegistry.findClusterById(IdentityProvider.ServiceType.OPENSHIFT)).thenReturn(Optional.of(cluster));
        when(request.getHeader(CouldServiceProducer.OPENSHIFT_AUTHORIZATION_HEADER)).thenReturn(null);
